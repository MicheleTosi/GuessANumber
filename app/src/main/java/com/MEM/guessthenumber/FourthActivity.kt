...
import com.MEM.guessthenumber.databinding.ActivityThirdBinding
...
class FourthActivity : AppCompatActivity() {
    ...
    inner class Holder(binding: ActivityThirdBinding, bool: Boolean) {
        private var state = 0
        ...
        private fun startGame(){
            ...
            gameStarted = true
            gan.new()
            mybinding.ivState.setImageResource(R.drawable.ic_wait)
            enableButtons()
        }
        ...
        inner class ActionClick :View.OnClickListener{
            override fun onClick(view: View?) {
                ...
                when(view.id){
                    R.id.btnCanc -> {
                        mybinding.tvGAN.text = mybinding.tvGAN.text.toString().dropLast(1)
                        mybinding.btnOK.isEnabled = mybinding.tvGAN.text.toString()!=""
                        if (mybinding.tvGAN.text.toString() !="")
                        if (mybinding.tvGAN.text.toString().toInt() in 1..maxValue ) {
                            enableButtons()
                        }
                    }
